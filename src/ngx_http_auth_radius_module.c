...
static ngx_int_t
ngx_send_radius_request(ngx_http_request_t *r,
...
ngx_send_radius_request(ngx_http_request_t *r, radius_req_queue_node_t* prev_req)
{
    ...
    radius_req_queue_node_t* n;
    // TODO: ! log
    n = radius_send_request(mcf->servers, prev_req, &user, &passwd, clcf->error_log);
    if (n == NULL) {
        abort(); // TODO
    }
    ...
    ngx_add_timer(r->connection->read, lcf->radius_timeout);
    ...
    if (c == NULL) {
        ...
        c = ngx_get_connection(rs->sockfd, mcf->log);
        if (c == NULL) {
...
ngx_http_auth_radius_handler(ngx_http_request_t *r)
{
    ...
    ngx_http_auth_radius_main_conf_t* mcf;
    mcf = ngx_http_get_module_loc_conf(r, ngx_http_auth_radius_module);

    if (mcf->realm.data == NULL || mcf->realm.len == 0)
        return NGX_DECLINED;
    ...
    if (ctx == NULL) {
        ...
        ctx->attempts = mcf->radius_attempts;
        ctx->done = 0;
        ctx->accepted = 0;
        ...
        // TODO: set timer and add timer_handler
...
ngx_http_auth_radius_create_loc_conf(ngx_conf_t *cf)
{
    ...
    mcf->servers = NULL;
    mcf->realm.data = NULL;
    mcf->log = NGX_CONF_UNSET_PTR;
    mcf->radius_attempts = NGX_CONF_UNSET;
    mcf->radius_timeout = NGX_CONF_UNSET_MSEC;
                                        ...
                                        void *conf)
{
    ...
    mcf->radius_timeout = ngx_atoi(value[1].data, value[1].len);
                                         ...
                                         void *conf)
{
    ...
    mcf->radius_attempts = ngx_atoi(value[1].data, value[1].len);
    ...
    if (ngx_parse_url(cf->pool, &u) != NGX_OK) {
        if (u.err) {
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                               "ngx_http_auth_radius_set_radius_server: %s \"%V\"",
                               u.err, &u.url);
        }
